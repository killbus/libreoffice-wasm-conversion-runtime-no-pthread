...
const USAGE = `Usage:
  node scripts/release-runtime/browser-worker-gate.mjs \\
    ...
    --out <evidence.json> \\
    [--chrome <executable>]`
const FLAGS = new Set(['native-root', 'wrapper-root', 'fixture', 'out', 'chrome'])
const MIME = {
...
async function buildAssets(args) {
  ...
  const paths = new Map([
    ...
    ['/dist/browser.worker.global.js', resolve(wrapperRoot, 'dist/browser.worker.global.js')],
...
function gateHtml() {
  return Buffer.from(`<!doctype html><meta charset="utf-8"><title>runtime gate</title>
<script type="module">
...
async function main(argv) {
  ...
  try {
    ...
    page.on('pageerror', (error) => {
      consoleMessages.push({ type: 'pageerror', text: error.message.slice(0, 1000) })
    })
    ...
    gateError = error instanceof Error ? error.stack ?? error.message : String(error)
  } finally {
    await browser?.close().catch(() => {})
    await new Promise((resolveClose) => server.close(resolveClose))
  }

  const standaloneWorkerRequests = [...new Set(
    [...serverRequests, ...browserRequests.map(({ path }) => path)].filter(
      (path) => path.split('/').at(-1) === 'soffice.worker.js'
    )
  )]
  const sofficeJsRequests = serverRequests.filter((path) => path === '/wasm/soffice.js').length
  const passed =
    !gateError &&
