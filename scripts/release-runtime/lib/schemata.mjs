...
function expectCommit(value, label) {
  ...
  if (!COMMIT_PATTERN.test(normalized)) {
    throw new SchemaError(`${label} must be an exact 40-character hex Git commit`)
  }
...
function expectRunId(value) {
  ...
  if (!RUN_ID_PATTERN.test(normalized)) {
    throw new SchemaError('GitHub Actions run ID must contain only decimal digits')
  }
  return normalized
}
...
function expectAbi(value) {
  ...
  if (!ABI_PATTERN.test(normalized)) {
    throw new SchemaError('native ABI must be a non-empty lowercase identifier')
  }
  return normalized
}
...
export function normalizeProvenance(provenance) {
  ...
  if (!native || !wrapper) {
    throw new SchemaError('provenance.native and provenance.wrapper are required')
  }
...
export function expectAsset(asset, { requireSource = true } = {}) {
  ...
  if (requireSource) {
    const sourceRoot = expectString(asset?.sourceRoot, `sourceRoot for ${path}`)
...
export function validateFrozenSpec(value) {
  ...
  const candidateId = expectSha256(value.candidateId, 'frozen spec candidateId')
  ...
  if (!Array.isArray(value.assets) || value.assets.length !== 8) {
    throw new SchemaError('frozen spec must declare exactly eight runtime assets')
  }
...
export function validateCandidateManifest(value) {
  if (
    ...
    throw new SchemaError(
      `candidate manifest kind/schema mismatch'`
    )
  }
  if (value.releaseQualified !== false) {
    throw new SchemaError(
      'candidate manifest releaseQualified must be false'
    )
  }
  if (
    'timestamp' in value ||
...
function expectControlRecordHead(value, expectedKind, label) {
  ...
  if (value.schemaVersion !== CONTROL_SCHEMA_VERSION) {
    throw new SchemaError(`${label} schemaVersion must be ${CONTROL_SCHEMA_VERSION}`)
  }
}
...
export function validateStagingReport(value) {
  ...
  if (value.releaseQualified !== undefined && value.releaseQualified !== false) {
...
export function validateAcceptanceReceipt(value) {
  ...
  if (value.decision !== 'accepted' && value.decision !== 'rejected') {
    throw new SchemaError('acceptance receipt decision must be accepted/rejected')
  }
  ...
  if (!Array.isArray(value.gateResults)) {
    throw new SchemaError('receipt gateResults must be a list')
  }
  return value
}
...
export function validateReleaseManifest(value) {
  ...
  expectSha256(value.candidateManifestSha256, 'release manifest candidate hash')
